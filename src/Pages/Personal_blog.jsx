...
const Personal_blog = () => {
    ...
    const handleSortChange = (e) => {
        ...
        <div className="container">
            ...
            <div class="pt-3 ">
                <h1 class="d-flex justify-content-center py-3">This is Personal Blog</h1>
                <h5>
                    <ol class="list-style-number d-flex justify-content-around py-4">
                        {sortedPosts.map((post) => (
                            <div key={post.id} class="post p-5  mx-2 rounded border bg-light ">
                                <Link class="text-decoration-none text-dark " to={post.isauthenticate ? `/blogs/personal_blog/${post.id}` : post.role === "superuser" ? '/superuser' : post.role === "admin" ? '/admin' : '/'}>{post.title}</Link>
                            </div>
                        ))}
                    </ol>
                </h5>
